...
def run_iter(agent, optimizer):
    ...
    # use agent current model to select action
    current_next_state_q_s = agent.model(batch_next_state_v)

    _, next_state_action = torch.max(current_next_state_q_s, dim=1, keepdim=True)
    ...
    max_next_q_s = all_ideal_next_q_s.gather(1, next_state_action)
