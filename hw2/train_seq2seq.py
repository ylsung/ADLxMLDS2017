import numpy as np
import random
from model_base import RNNEncoder, attnRNNDecoder
import torch
...
def train_iteration(data_generator, decoder, encoder, optimizer_en, optimizer_de, criterion, word2num, num2word, ratio):
    ...
    for i in range(1, batch_label.shape[1]):
        ...
        pre_word = schedule_sampling(pred_word[:, i - 1], batch_label[:, i - 1], 1.0)
...
def predict(data, decoder, encoder, batch_size, word2num, max_length):
    ...
    while not end_epoch:
        ...
        if end < data.shape[0]:
            ...
            h, c = encoder.init_hidden(batch_size)
            end_epoch = 0
            overlap = batch_size
        ...
        if end < data.shape[0]:
            ...
            for i in range(1, max_length):
                ...
                for j in range(len(candidates[0])):
                    ...
                    beam_search(prob, pred, candidates, beam_output, h, c, batch_size, beam_size, j, i)
            ...
            for i in range(batch_data.shape[0]):
                candidates.append([(0.0, deepcopy(pred_word[i]), deepcopy(h[:, i, :]).unsqueeze(1), 
                    deepcopy(c[:, i, :]).unsqueeze(1))])
                beam_output.append([])
...
def train(args, train_data, label_array, str_length_array, caption_length_array, name_array, word2num, num2word, valid_dict, max_length):
    ...
    model_info_path = os.path.join('model', args.save, 'model_info.pickle')
    ...
    for epoch in range(MaxEpoch):
        ratio = 1.0 - float(epoch) / float(MaxEpoch)
        loss, _, __ = train_epoch(data_generator, valid_data_generator, decoder, encoder, optimizer_en,
        ...
        if epoch % 100 == 99:
            decoder_path = os.path.join('model', args.save, 'decoder_%d.pth' % epoch)
            encoder_path = os.path.join('model', args.save, 'encoder_%d.pth' % epoch)
...
def test(args, test_data, test_name, word2num, num2word, max_length):
    model_id = 99
    decoder_path = os.path.join('model', args.load, 'decoder_%d.pth' % model_id)
    encoder_path = os.path.join('model', args.load, 'encoder_%d.pth' % model_id)
    ...
    model_info_path = os.path.join('model', args.load, 'model_info.pickle')
    ...
    test_pred = predict_BeamSearch(test_data, decoder, encoder, batch_size, word2num, max_length, 1)
